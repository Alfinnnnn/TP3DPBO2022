...
def inputs():
    ...
    # Input 3
    options = ["Filsafat Meme", "Sastra Mesin", "Teknik Kedokteran", "Pendidikan Gaming", "Teknik Bisnis", "Agama"]
    ...
    # Input 4
    jenisKelamin = [("Laki-Laki", "Laki-Laki"), ("Perempuan", "Perempuan")]
    label5 = Label(dframe, text="Jenis Kelamin").grid(row=3, column=0, sticky="w")
    jKelamin = StringVar();
    jKelamin.set("Laki-Laki")
    i = 0
    for text, jkel, in jenisKelamin:
        input5 = Radiobutton(dframe, text=text, variable=jKelamin, value=jkel).grid(row=3+i, column=1, padx=20, pady=10, sticky='w')
        i=i+1

    #input 5
    hobi = ["Ngahuleng", "Membaca", "Ngaji", "Rebahan", "Ngoding", "Makan"]
    label6= Label(dframe, text="Hobi").grid(row=5, column=0, sticky='w')
    hobbi = ttk.Combobox(dframe, value=hobi)
    hobbi.grid(row=5, column=1, padx=20, pady=10, sticky='w')
    hobbi.current(0)
    ...
    # Submit Button
    btn_submit = Button(frame2, text="Submit Data", anchor="s", command=lambda:[insertData(top, input_nama, input_nim, input_jurusan, jKelamin, hobbi), top.withdraw()])
...
def fasilitas():
    ...
    img1 = ImageTk.PhotoImage(Image.open('img/library.jpg'))
    img2 = ImageTk.PhotoImage(Image.open('img/canteen.jpg'))
    img3 = ImageTk.PhotoImage(Image.open('img/classroom.jpg'))
    img4 = ImageTk.PhotoImage(Image.open('img/mosque.jpg'))
    ...
    # next btn
    if len(image_list) == 1 :
        btn_next = Button(buttonFrame, text=">>", anchor="s", command=lambda: imgNext(idx+1,  image_list,  dframe, buttonFrame, btn_prev, btn_next), state=DISABLED)
    else :
